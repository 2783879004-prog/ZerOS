...
class TaskbarManager {
    ...
    static init() {
        ...
        // 渲染任务栏
        TaskbarManager._renderTaskbar(taskbar);
    ...
    static _renderTaskbar(taskbar) {
        ...
        // 获取常显程序列表
        let alwaysShowPrograms = [];
        if (typeof ApplicationAssetManager !== 'undefined' && typeof ApplicationAssetManager.getAlwaysShowPrograms === 'function') {
            try {
                alwaysShowPrograms = ApplicationAssetManager.getAlwaysShowPrograms();
            } catch (e) {
                KernelLogger.warn("TaskbarManager", `获取常显程序列表失败: ${e.message}`);
            }
        }
        ...
        // 先添加常显程序（无论是否运行都显示）
        for (const program of alwaysShowPrograms) {
        ...
        for (const runningProgram of runningPrograms) {
            ...
            if (existing) {
                ...
                if (instances.length === 1) {
                    ...
                    existing.pid = instances[0].pid; // 使用第一个实例的PID作为默认
                }
            } else {
                // 如果程序不在常显列表中，也添加到任务栏（仅在运行时显示）
    ...
    static async _loadSystemIconWithRetry(iconName, container, preserveSize = false, maxRetries = 50, retryDelay = 100) {
        ...
        if (!themeManager && typeof DependencyConfig !== 'undefined') {
            try {
                ...
                await Dependency.waitLoaded("../kernel/process/themeManager.js", 5000); // 最多等待 5 秒
                // 重新检查 ThemeManager
                if (typeof POOL !== 'undefined' && typeof POOL.__GET__ === 'function') {
                    try {
                        themeManager = POOL.__GET__("KERNEL_GLOBAL_POOL", "ThemeManager");
...
if (typeof DependencyConfig !== 'undefined') {
    DependencyConfig.publishSignal("../kernel/process/taskbarManager.js");
}
