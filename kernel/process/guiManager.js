...
class GUIManager {
        ...
        if (themeManager) {
            try {
                ...
                if (currentTheme && currentTheme.colors) {
                    ...
                    windowElement.style.backgroundColor = windowBg;
                ...
                KernelLogger.warn("GUIManager", `设置窗口初始背景色失败: ${e.message}`);
            }
        }
    ...
    static closeMainWindowAndChildren(pid) {
        ...
        for (const childWindow of childWindows) {
            GUIManager._closeWindow(childWindow.windowId, true);
        }
            ...
            EventManager.registerResizer(
                ...
                (e) => {
                    ...
                    if (guiContainer) {
                        ...
                        // 计算新的top位置（向上扩展）
                        const heightDelta = windowInfo.windowState.resizeStartHeight - newHeight;
                        const newTop = Math.max(0, relativeTop - heightDelta);
                        ...
                        // 计算最大尺寸（确保窗口不超出视口）
                        const maxWidth = Math.max(minWidth, viewportWidth - rect.left);
                        const maxHeight = Math.max(minHeight, rect.top + windowInfo.windowState.resizeStartHeight);
                        ...
                        // 计算新的top位置（向上扩展）
                        const heightDelta = windowInfo.windowState.resizeStartHeight - newHeight;
                        const newTop = Math.max(0, windowInfo.windowState.resizeStartTop - heightDelta);
    ...
    static _updateAllWindowsStyles() {
        ...
        if (currentStyle.styles && currentStyle.styles.window) {
            ...
            allWindows.forEach(window => {
                ...
                window.style.backdropFilter = windowStyles.backdropFilter;
                window.style.webkitBackdropFilter = windowStyles.backdropFilter;
        ...
        if (currentStyle.styles && currentStyle.styles.taskbar) {
            ...
            if (taskbar) {
                ...
                taskbar.style.backdropFilter = taskbarStyles.backdropFilter;
                taskbar.style.webkitBackdropFilter = taskbarStyles.backdropFilter;
