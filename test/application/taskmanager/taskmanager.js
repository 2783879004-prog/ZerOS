    ...
    const TASKMANAGER = {
        ...
        _createTitleBar: function() {
            ...
            titleBar.style.cssText = `
                height: 48px;
                background: linear-gradient(180deg, rgba(108, 142, 255, 0.1) 0%, rgba(108, 142, 255, 0.05) 100%);
        ...
        _updateProcessDetail: function(pid) {
            ...
            if (process.actions && process.actions.length > 0) {
                const actionsInfo = this._createInfoSection('行为记录', [
                    { label: '记录数量', value: process.actions.length.toString() },
                    { label: '最后操作', value: process.actions[process.actions.length - 1].action || '未知' },
                    { label: '最后操作时间', value: process.actions[process.actions.length - 1].timestamp ? 
                        new Date(process.actions[process.actions.length - 1].timestamp).toLocaleString() : '未知' },
                ]);
                detail.appendChild(actionsInfo);
            }
