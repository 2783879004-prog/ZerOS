...
function escapeHtml(s){
                ...
                if(ev.key === 'Tab'){
                    ...
                    try{
                        if(isFirstToken){
                            ...
                            const COLL = safePoolGet('KERNEL_GLOBAL_POOL', dirToList.split('/')[0]);
                            const nodes = (COLL.list_dir(dirToList) || []).map(n => n.name + '/');
                            const files = (COLL.list_file(dirToList) || []).map(f => f.name);
        ...
        switch(cmd){
            ...
            case 'cd':
                if(payload.args.length < 2 || payload.args.length > 3){
                    ...
                    // 检查路径是否需要解析
                    // if(newPath.includes('/')){
                    //     payload.write(`cd: changing to path "${newPath}" not supported in this demo`);
                    //     return;
                    // }
                    // 兼容..返回上级目录
                    if(newPath === '..'){
                    ...
                    const COLL = safePoolGet('KERNEL_GLOBAL_POOL', resolved.split("/")[0]);
                    if(!COLL.hasNode(resolved)){
            ...
            case 'markdir':
                ...
                // 创建
                COLL.create_dir(parentPath, newName);
                // payload.write(`markdir: directory "${dirName}" created`);
                break;
            case 'markfile':
                if(payload.args.length < 2 || payload.args.length > 3){
                ...
                // 调用创建
                COLL2.create_file(parent, fileObj);
                break;
            case 'ls':
                {
                    ...
                    const COLL3 = safePoolGet('KERNEL_GLOBAL_POOL', targetPath.split("/")[0]);
                    
                    // 辅助函数：获取文件类型和颜色
                    const getFileTypeInfo = (filename, fileObj) => {
                        let fileType = 'TEXT';
                        let typeColor = '#e6e6e6'; // 默认白色
                        ...
                        } else if (fileObj && fileObj.fileType !== undefined) {
                            // 如果文件对象有 fileType 属性，尝试使用
                            const typeNames = {
                                [FileType?.GENRE?.TEXT || 0]: 'TEXT',
                                [FileType?.GENRE?.IMAGE || 1]: 'IMAGE',
                                [FileType?.GENRE?.CODE || 2]: 'CODE',
                                [FileType?.GENRE?.BINARY || 3]: 'BINARY',
                                [FileType?.GENRE?.JSON || 4]: 'JSON',
                                [FileType?.GENRE?.XML || 5]: 'XML',
                                [FileType?.GENRE?.MARKDOWN || 6]: 'MARKDOWN',
                                [FileType?.GENRE?.CONFIG || 7]: 'CONFIG',
                                [FileType?.GENRE?.DATA || 8]: 'DATA',
                            };
                            fileType = typeNames[fileObj.fileType] || 'TEXT';
                        }
                    ...
                    // 如果目标本身是一个文件，单列该文件
                    if(!COLL3.hasNode(targetPath)){
                        // 可能是文件
                        const parent = targetPath.split('/').slice(0, -1).join('/') || targetPath.split('/')[0];
                        const fname = targetPath.split('/').pop();
                        const parentNode = COLL3.getNode(parent);
                        if(!parentNode){ payload.write(`ls: cannot access '${targetArg}': No such file or directory`); return; }
                        const fileEntry = parentNode.attributes && parentNode.attributes[fname] ? { name: fname, size: parentNode.attributes[fname].fileSize || 0, path: parent + '/' + fname, file: parentNode.attributes[fname] } : null;
                        if(!fileEntry){ payload.write(''); return; }
                        if(!longFlag){ payload.write(fileEntry.name); return; }
                        // long format for single file
                        const fmtDate = (ts) => { if(!ts) return '------------'; const d=new Date(ts); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')} ${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}` };
                        const perms = '-rw-r--r--';
                        const size = fileEntry.size || 0;
                        const mtime = (fileEntry.file && (fileEntry.file.fileModifyTime || fileEntry.file.fileCreatTime)) ? (fileEntry.file.fileModifyTime || fileEntry.file.fileCreatTime) : null;
                        const owner = payload.env.user || 'root';
                        const { fileType, typeColor } = getFileTypeInfo(fname, fileEntry.file);
                        
                        // 使用 HTML 渲染，带颜色标记
                        payload.write({
                            html: `${perms} ${String(1).padStart(3,' ')} <span style="color: #4a9eff;">${owner}</span> <span style="color: ${typeColor};">${fileType.padEnd(8)}</span> ${String(size).padStart(8,' ')} ${fmtDate(mtime)} ${fileEntry.name}`,
                        });
                        return;
                    }

                    const nodes = (COLL3.list_dir(targetPath) || []);
                    const files = (COLL3.list_file(targetPath) || []);
                    if(!longFlag){
                        if(nodes.length === 0 && files.length === 0){ payload.write(''); return; }
                        let outParts = [];
                        for(const n of nodes) outParts.push(n.name + '/');
                        for(const f of files) outParts.push(f.name);
                        payload.write(outParts.join('\t'));
                        return;
                    }

                    // long listing 格式化（带表头）
                    const owner = payload.env.user || 'root';
                    payload.write({
                    ...
                    for(const n of nodes){
                        ...
                        const size = (n.node && n.node.children) ? n.node.children.size : 0;
                        const mtime = (n.node && n.node.__meta && n.node.__meta.mtime) ? n.node.__meta.mtime : null;
                        const { fileType, typeColor } = getFileTypeInfo(name, null);
                        rows.push({
                            html: `${perms} ${String(1).padStart(3,' ')} <span style="color: #4a9eff;">${owner}</span> <span style="color: ${typeColor};">DIR</span> ${String(size).padStart(8,' ')} ${fmtDate(mtime)} <span style="color: #00ff00;">${name}/</span>`,
                        });
                    }
                    ...
                    for(const f of files){
                        const name = f.name;
                        const perms = '-rw-r--r--';
                        const size = f.size || (f.file && f.file.fileSize) || 0;
                        const mtime = (f.file && (f.file.fileModifyTime || f.file.fileCreatTime)) ? (f.file.fileModifyTime || f.file.fileCreatTime) : null;
                        const { fileType, typeColor } = getFileTypeInfo(name, f.file);
                        rows.push({
                            html: `${perms} ${String(1).padStart(3,' ')} <span style="color: #4a9eff;">${owner}</span> <span style="color: ${typeColor};">${fileType.padEnd(8)}</span> ${String(size).padStart(8,' ')} ${fmtDate(mtime)} ${name}`,
                        });
                    }
                    
                    if(rows.length === 0){ payload.write(''); return; }
                    for(const r of rows) payload.write(r);
                }
                break;
            case 'cat':
                // 查看文件，支持相对或绝对路径，支持 -md 参数渲染Markdown
                if(payload.args.length < 2){ payload.write('cat: missing operand'); return; }
                {
                    const args = payload.args.slice(1);
                    ...
                    const fname = parts.pop();
                    const parent = parts.join('/') || parts[0];
                    const COLLc = safePoolGet('KERNEL_GLOBAL_POOL', full.split('/')[0]);
                    const nodeC = COLLc.getNode(parent);
                    if(!nodeC || !nodeC.attributes || !nodeC.attributes[fname]){ payload.write(`cat: ${fArg}: No such file`); return; }
                    const content = COLLc.read_file(parent, fname);
            ...
            case 'write':
                ...
                if(payload.args.length < 3){ payload.write('write: missing operand'); return; }
                {
                    ...
                    if(!nodeW.attributes[fname]){
                        ...
                        COLLw.create_file(parent, fileObj);
                    ...
                    try{
                        ...
                        if(typeof COLLw.write_file === 'function'){
                            COLLw.write_file(parent, fname, content, mode);
                        }else{
            ...
            case 'tree':
                {
                    ...
                    const COLL = safePoolGet('KERNEL_GLOBAL_POOL', targetPath.split("/")[0]);
                    if (!COLL) {
                    ...
                    // 检查目标是否存在
                    if (!COLL.hasNode(targetPath)) {
                        payload.write(`tree: '${targetPath}' is not a directory`);
                        return;
                    }
                    ...
                    // 递归构建树结构
                    const buildTree = (path, prefix = '', isLast = true, depth = 0) => {
                        if (maxDepth >= 0 && depth > maxDepth) {
                            return;
                        }
                        
                        const nodes = COLL.list_dir(path) || [];
                        const files = COLL.list_file(path) || [];
                        const allItems = [];
                        
                        // 先添加目录，再添加文件
                        for (const node of nodes) {
                            allItems.push({ ...node, isDir: true });
                        }
                        for (const file of files) {
                            allItems.push({ ...file, isDir: false });
                        }
                        ...
                        for (let i = 0; i < allItems.length; i++) {
                            ...
                            if (item.isDir) {
                                ...
                                // 递归处理子目录
                                if (COLL.hasNode(dirPath)) {
                                    buildTree(dirPath, nextPrefix, isLastItem, depth + 1);
                                }
                            } else {
                                // 文件
                                const fileName = item.name;
                                const { fileType, typeColor } = getFileTypeInfo(fileName, item.file);
                                const size = item.size || (item.file && item.file.fileSize) || 0;
                    ...
                    const countItems = (path, depth = 0) => {
                        if (maxDepth >= 0 && depth > maxDepth) return;
                        
                        const nodes = COLL.list_dir(path) || [];
                        const files = COLL.list_file(path) || [];
                        
                        dirCount += nodes.length;
                        fileCount += files.length;
                        
                        for (const node of nodes) {
                            const nodePath = node.path || (path + '/' + node.name);
                            if (COLL.hasNode(nodePath)) {
                                countItems(nodePath, depth + 1);
                            }
                        }
                    };
                    
                    countItems(targetPath);
            ...
            case 'check':
                {
                    ...
                    check('FileFramework', typeof FileFramework !== 'undefined',
                        typeof FileFramework !== 'undefined' ? '已加载' : '未加载');
                    if (typeof FileFramework !== 'undefined') {
            ...
            case 'diskmgr':
                ...
                if (!listMode) {
                    // 确保 diskSeparateMap 是最新的（从 POOL 同步）
                    if (Disk.diskSeparateMap) {
                        // 尝试从 POOL 获取所有分区，确保 diskSeparateMap 包含所有分区
                        if (typeof POOL !== 'undefined' && typeof POOL.__GET__ === 'function') {
                            try {
                                ...
                                for (const partitionName of knownPartitions) {
                                    if (!Disk.diskSeparateMap.has(partitionName)) {
                                        try {
                                            const coll = POOL.__GET__("KERNEL_GLOBAL_POOL", partitionName);
                                            if (coll) {
                    ...
                    Disk.diskSeparateSize.forEach((size, name) => {
                        const used = Disk.diskUsedMap.get(name) || 0;
                        const free = Disk.diskFreeMap.get(name) || (size - used);
                        payload.write(`${name} : total=${size} used=${used} free=${free}`);
                    });
                    payload.write('');
                    ...
                    disksToShow.forEach(diskName => {
                        // 首先尝试从 diskSeparateMap 获取
                        let COLL = null;
                        if (Disk.diskSeparateMap && Disk.diskSeparateMap.has(diskName)) {
                            COLL = Disk.diskSeparateMap.get(diskName);
                        }
                        
                        // 如果从 diskSeparateMap 获取失败，尝试从 POOL 获取
                        if (!COLL) {
                            COLL = safePoolGet('KERNEL_GLOBAL_POOL', diskName);
                            // 如果从 POOL 获取成功，也更新 diskSeparateMap（保持一致性）
                            if (COLL && Disk.diskSeparateMap) {
                                Disk.diskSeparateMap.set(diskName, COLL);
                                // 更新缓存
                                Disk._diskSeparateMapCache = Disk.diskSeparateMap;
                            }
                        }
                        ...
                        if (!COLL) {
                            payload.write(`diskmanger: Disk ${diskName} not found`);
                            return;
                        }
                        
                        const totalSize = Disk.diskSeparateSize.get(diskName) || 0;
                        const usedSize = Disk.diskUsedMap.get(diskName) || 0;
                        const freeSize = Disk.diskFreeMap.get(diskName) || (totalSize - usedSize);
                        ...
                        const calculateDirSize = (dirPath) => {
                            const node = COLL.getNode(dirPath);
                            if (!node) return 0;
                            
                            let size = 0;
                            
                            // 计算文件大小
                            const files = COLL.list_file(dirPath) || [];
                            files.forEach(file => {
                                size += file.size || (file.file && file.file.fileSize) || 0;
                            });
                            
                            // 递归计算子目录大小
                            const dirs = COLL.list_dir(dirPath) || [];
                            dirs.forEach(dir => {
                                const childPath = dir.path || (dirPath + '/' + dir.name);
                                size += calculateDirSize(childPath);
                            });
                        ...
                        const collectItems = (path) => {
                            ...
                            const files = COLL.list_file(path) || [];
                            files.forEach(file => {
                                const fileSize = file.size || (file.file && file.file.fileSize) || 0;
                                items.push({
                                    path: file.path,
                                    name: file.name,
                                    type: 'file',
                            ...
                            // 递归处理子目录
                            const dirs = COLL.list_dir(path) || [];
                            dirs.forEach(dir => {
                                const dirPath = dir.path || (path + '/' + dir.name);
                                const subDirSize = calculateDirSize(dirPath);
                                items.push({
                                    path: dirPath,
                                    name: dir.name + '/',
                                    type: 'dir',
                        ...
                        payload.write('');
                        payload.write(`Total items: ${items.length} (${items.filter(i => i.type === 'dir').length} directories, ${items.filter(i => i.type === 'file').length} files)`);
                    });
                }
                break;
            case 'eval':
    ...
    if (typeof DependencyConfig !== 'undefined') {
        DependencyConfig.publishSignal("../../test/application/terminal/terminal.js");
    }
