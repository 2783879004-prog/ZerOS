    ...
    class Vim {
        ...
        _loadStateFromStack() {
            ...
            openFile(filepath) {
                ...
                // 解析路径
                const resolved = this._resolvePath(this.terminalEnv.cwd, filepath);
                ...
                try {
                    ...
                    if (typeof POOL !== 'undefined' && typeof POOL.__GET__ === 'function') {
                        COLL = POOL.__GET__('KERNEL_GLOBAL_POOL', root);
                    } else if (typeof window !== 'undefined' && window.POOL && typeof window.POOL.__GET__ === 'function') {
                        COLL = window.POOL.__GET__('KERNEL_GLOBAL_POOL', root);
                    }
                } catch (e) {
                    console.error('Vim: Error accessing POOL', e);
                }
                
                if (!COLL) {
                    this.terminalWrite('Vim: Cannot access file system');
                    return;
                }
        ...
        saveFile() {
            ...
            try {
                ...
                if (!node || !node.attributes || !node.attributes[fileName]) {
                    // 文件不存在，需要创建
                    // 这里简化处理，实际应该创建文件
                    this.terminalWrite('Vim: File does not exist, cannot save');
                    return;
                }
                ...
                try {
                    if (typeof FileType !== 'undefined') {
                        ...
                        FileTypeRef = window.POOL.__GET__('TYPE_POOL', 'FileType');
                    }
                } catch (e) {
                    console.error('Vim: Error accessing FileType', e);
                }
                
                const writeMode = FileTypeRef && FileTypeRef.WRITE_MODES ? FileTypeRef.WRITE_MODES.OVERWRITE : 0;
                COLL.write_file(parentPath, fileName, content, writeMode);
                this.unsavedChanges = false;
